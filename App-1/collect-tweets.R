...
handles_orig <- read.csv("./App-1/candidate_twitter_handles.csv",
                         na.strings = "",
...
candidate_timelines_select <- candidate_timelines %>% 
  select(user_id:text, hashtags, media_url, media_type) %>% 
  mutate(screen_name = tolower(screen_name)) 

#join to get the political party

candidate_tweets <- 
candidate_timelines_select %>% 
  left_join(handles_only, by = c("screen_name" = "twitter_handle"))

saveRDS(candidate_timelines, file = "./App-1/full_candidate_tl.rds")
saveRDS(candidate_timelines_select, file = "./App-1/short_candidate_tl.rds")

saveRDS(candidate_timelines, file = "./App-1/short_candidate_tl_joined.rds")
