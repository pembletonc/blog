...
The Hadley's drill a point down: if you've cut and paste something more than twice - it's time to write a function!  
...
All of these matter! Give a function a descriptive name, have argument (e.g. what goes into the function's ()) names which make sense, and ensure the argument order is logical. Arugment order should be with data arguments first, and detail arguments second.
...
Why learn to write functions programmatically? Because in many cases (and pretty much every case I've seen professionally) functions often need to be flexible enough to take multiple arguments to achieve a desired result, or meet multiple needs. 

Functional programming is first introduced by showing how to create a function which takes another function as an argument. So instead of:
...
There are some interesting ways of how ```.f``` can be specified within map.

Defining an anonymous function within it:

```{r}
map(df, function(x) sum(is.na(x)))
```

Or, smoother yet, defining an anonymous function as a formula:

```{r}
map(df, ~ sum(is.na(.)))
```

This is very useful, and I've used this approach in multiple occasions. In most cases, it has been to run a function over a list of dataframes or vectors:
...
The walk family of functions in purrr are designed as map functions for side-effects of functions such as printing outputs, plotting, and saving files to disk. In the very useful example provided, the map + walk2 example allows me to create plots for each grouped variable and apply unique path names to each.
...
### Dealing with type-unstable functions
