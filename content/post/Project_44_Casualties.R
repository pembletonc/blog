...
#create a function to calculate the number of casualities from each
#division as i) proportion of total; ii) a proportion of the division 

View(cas1)

prop_function <- function(data, var, ...){
  ...
  data %>% 
    group_by(!!!group_vars) %>%
    mutate(daily_mean = mean(count(!!!var), na.rm = TRUE))
}

prop_function(cas1, Division, Division)
